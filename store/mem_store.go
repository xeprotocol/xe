...
// MemStore is an in-memory implementation of core.Store backed by Go maps.
// It also implements ConflictStore, VoteStore, QuorumStore, DelegationStore,
// and DelegationIterator so that test ledgers exercise the same code paths as
// BadgerStore-backed ledgers.
type MemStore struct {
	...
	// ConflictStore
	conflicts map[string]*core.Conflict // key: account + "|" + previousHash
	staged    map[string]*core.Block    // key: block hash

	// VoteStore
	votes map[string]*core.Vote // key: account + "|" + prev + "|" + repPubKey

	// QuorumStore
	blockStatus map[string]core.BlockStatus // key: block hash
	finalHeight map[string]uint64           // key: account

	// FinalVoteStore: write-once commit-lock (#526). key: account + "|" + previous → hash
	finalVotes map[string]string

	// DelegationStore
	delegations map[string]string // account → representative

	// LeaseStore
	leases map[string]*core.Lease // key: lease hash

	// CertificateStore (#630)
	certificates map[string][]byte // provider account → certificate JSON

	// ReputationStore
	reputation map[string]*core.ReputationAggregate // account → aggregate

	// KeysetStore
	keysets map[string]*core.Keyset // account → keyset

	// accountKeys is the single-key credential registry (#829): account address
	// → hex ed25519 public key, declared by the account's opening block.
	accountKeys map[string]string

	// StateChainStore
	stateBlocks map[uint64]*statechain.Block // key: block.Index (genesis index 0 is not stored)
}
...
// pending add/delete, and reject-status, all under one lock acquisition.
// Implements core.AtomicUndoStore (#570/H5).
...
// applyUndoLocked applies a block rollback's writes. Caller must hold m.mu.
// Factored out of CommitUndo so the #622 spanning cascade can apply several
// undos plus the winner commit under ONE lock acquisition. (#570/H5)
func (m *MemStore) applyUndoLocked(u *core.BlockUndo) error {
	if u.DeleteAccount {
		// #701: the undo emptied the chain — delete the account record so it
		// disappears from AllFrontiers (and /accounts), matching a fresh
		// rebuild-from-chain. A winner commit in the same cascade re-creates the
		// keys afterwards if it re-adds this account (open-conflict swap).
	...
	// Restore the delegation pointer under the same lock acquisition (#597):
	// rebuildDelegation reads it, so it must move with the chain truncation.
	...
	// Lease/keyset side-effect reversal (#570/C1), same transaction.
...
// Sync is a no-op for the in-memory store: its writes are already durable for
// the process lifetime. Implements core.Syncer. (#570/H1)
func (m *MemStore) Sync() error {
	return nil
}

// AllFrontiers returns a copy of the full account → frontier-hash map.
...
// CommitBlock atomically writes a block, chain, frontier, and pending
// send create/delete under a single lock acquisition.
...
// applyCommitLocked applies a block commit's writes (including the #570/C3
// lease-state CAS precondition). Caller must hold m.mu. Factored out of
// CommitBlock so the #622 spanning cascade can apply the winner commit in the
// same critical section as the undos.
func (m *MemStore) applyCommitLocked(c *core.BlockCommit) error {
	// Lease-state compare-and-swap (#570/C3): checked before any mutation so
	// a conflicting commit is all-or-nothing.
	...
	// PutFrontier (skip for non-frontier swaps where FrontierHash is empty)
	if c.FrontierHash != "" {
		m.frontiers[c.Account] = c.FrontierHash
	}

	// AddPending (send path)
	if c.AddPending != nil {
		cp := *c.AddPending
		m.pending[cp.SendHash] = &cp
		if m.pendingByDest[cp.Destination] == nil {
	...
	// PutLease (lease / lease_accept path)
	if c.PutLease != nil {
		cp := *c.PutLease
		m.leases[cp.LeaseHash] = &cp
	}

	// SettleLease (lease_settle path)
	...
	// UnfulfillLease (lease_force_settle path, #488)
	...
	// Keyset (multisig_open / multisig_update path)
	if c.PutKeyset != nil {
		ks := *c.PutKeyset
	...
	// Account public key (#829): declared by the chain's opening block. Set —
	// not insert-only — so #424 key rotation needs no storage change.
	if c.PutAccountKey != "" {
...
// CommitCascade applies all undos (in order) then the winner commit under ONE
// lock acquisition — the in-memory mirror of the BadgerStore #622 spanning
// transaction. The lease-state CAS precondition (#570/C3) is validated BEFORE
// any undo mutates, so a CAS conflict leaves nothing applied (the mem store
// has no txn discard, so the all-or-nothing guarantee for the precondition is
// achieved by checking it first). The check runs against the EFFECTIVE
// POST-UNDO lease state, not the live map: BadgerDB's CAS reads inside the
// same txn AFTER the undos (read-your-writes), so an undo that restores the
// very state the winner expects — e.g. promoting an equivocated lease_accept,
// whose loser's undo restores the lease to `created` — must pass here too, or
// the two stores diverge. An ErrTxnTooBig-class failure cannot occur in
// memory, but the error is wrapped with the undo count for parity. (#622)
func (m *MemStore) CommitCascade(undos []*core.BlockUndo, commit *core.BlockCommit) error {
	m.mu.Lock()
	defer m.mu.Unlock()

	// Validate the winner commit's CAS precondition before any mutation,
	// against the post-undo state: the LAST undo touching the expected lease
	// (RestoreLease / DeleteLeaseHash) decides what the CAS will see.
...
// GetFinalVote returns the write-once commit-lock hash for a position (#526).
...
// PutFinalVoteIfAbsent records hash iff absent; never overwrites (#526).
...
// Keyed by certificate hash, not provider — see BadgerStore.PutCertificate (#816).
...
// DropLeaseForTesting removes a lease record while leaving the blocks that
// created it in place. There is no production delete path for leases — this
// exists so a test can inject the "derived state lost, chain intact" corruption
// that a supply auditor must refuse to guess its way past (#837).
...
// AllAccountKeys returns a copy of the full account → public-key map.
...
// State blocks are keyed by their own Index, mirroring BadgerStore's key→value
// layout. Get(i) returns the block with Index i; StateBlockCount is the number
// of stored blocks. The genesis block (index 0) is held by statechain.Chain and
// is not stored here. See #570/M9.
