...
// Terminal lease states. Bucketing must use the state field, not the legacy
// settled bool — a cancelled lease has settled=false but is not active (#755).
// The API always emits state (BackfillState runs on every lease read), so no
// settled-bool fallback is needed.
...
const state = {
  selfAddr: null,        // active wallet's address from vault, lower-cased
  providerRow: null,     // own row from /api/providers
  cert: null,            // own perf certificate (or null)
  active: [],            // leases where provider == self && state non-terminal
  past: [],              // leases where provider == self && state terminal
  pollTimer: null,
};
...
function hydrateFromSnapshot() {
  ...
  // Re-bucket rather than trust the snapshot's buckets: snapshots written
  // before #755 bucketed on the legacy settled bool, which strands cancelled
  // leases under active.
...
async function pollSafely() {
  ...
  try {
    await refreshAll();
    if (!state.providerRow) {
      // Account stopped advertising — stop polling, show gate.
