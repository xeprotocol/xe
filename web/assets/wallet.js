...
const state = {
  vault: null,         // { wallets: [...], active }
  passphrase: null,    // null = locked
  seed: null,          // Uint8Array(32) of active wallet
  keyPair: null,       // active wallet key pair
  networkId: '',
  chain: [],           // tail of the account chain; last element is the frontier
  chainTotal: 0,       // full chain length from the chain endpoint's total field
  balances: {},        // raw ledger balances (includes unfinalized inflows)
  spendable: {},       // finalized (irreversible) funds — the settlement figure (#528)
  pending: [],
  activity: [],
  pendingSeedBytes: null, // staged during create flow before passphrase save
};
...
// applyFeatures reads /features.json and hides controls for features the node
// hasn't wired up. The faucet button is hidden unless the node proxies a faucet
// service (cfg.FaucetTarget set), so it never offers a dead action.
...
async function routeStartup() {
  ...
  // Try the cached unlock from this tab session before prompting.
...
async function onMigrateSubmit(ev) {
  ...
  // Move legacy activity to per-wallet key.
...
async function onUnlockSubmit(ev) {
  ...
  state.keyPair = await loadKeyPair(seed);
  cacheUnlock(pass);
  await activateActive(); // refreshes UI based on active
}
...
async function activateActive() {
  ...
  // Decrypt active wallet's seed (cheap if same passphrase).
  try {
    state.seed = await decryptSeed(state.passphrase, w);
  } catch {
    // Different passphrase for this wallet (shouldn't happen — we use one passphrase).
    lockWallet();
  ...
  // Repair the cached identity if it predates #829, when a vault entry's
  // `address` was the raw public key. The seed is authoritative, so this is a
  // silent rewrite rather than an error — otherwise the locked UI, the
  // snapshot key and the DAO membership test all read a stale value.
...
function hydrateFromSnapshot() {
  ...
  if (typeof v.previous === 'string' && state.chain.length === 0) {
    // synthesize a minimal chain tail so renderPrevious sees the cached hash
...
function renderSwitcher() {
  ...
  if (ws.length <= 1) {
    switcher.hidden = false; // still show so + and lock are accessible
  } else {
...
// Removal itself lives on the manage page, which owns the seed-loss warning and
// the typed-name gate; this just deep-links to it with the active wallet primed.
...
async function onFaucet(ev) {
  ...
  try {
    ...
    const amt = result.amount != null ? formatAmount(result.amount, 'XUSD') : '100';
    showInfo(`faucet sent ${amt} XUSD · tx ${shortHash(result.tx_hash || '?')} · arriving as a pending receive`);
    logActivity('success', `faucet sent ${amt} XUSD · tx ${shortHash(result.tx_hash || '?')}`);
    await refreshAll();
  } catch (e) {
...
// stripDomainPrefix removes a leading "<domain>: " from an error message so
// callers can prepend their own action label without producing repeated
// prefixes like "faucet failed: faucet: ...".
...
async function refreshAll() {
  ...
  // #829: the address is the identity, the public key is the credential — show
  // both so a user pasting one into the wrong field notices.
  setText('wallet-pubkey', state.keyPair.pubKeyHex);
...
async function onSend(ev) {
  ...
  // The input is a decimal token amount (e.g. "1.5"); convert to micro-units.
  let amount;
  try {
  ...
  // Gate affordability on spendable (finalized funds, #528) — unfinalized
  // inflows are reversible and not safe to spend yet. The block itself is
  // built from the raw ledger balance below, which is what the node
  // validates the block's balance field against.
  ...
  // Byte-count the memo (UTF-8). maxlength on the input is rune-count, not
  // byte-count, so multibyte text could slip past it.
