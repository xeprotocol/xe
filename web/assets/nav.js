// nav.js — owns the global chrome (header + two-tier nav).
//
// Each page has empty <header id="xe-header"></header> and
// <nav id="xe-subnav" class="nav-secondary"></nav> placeholders. This module
// fills them, wires the search form, and polls /api/node for net/ver/status.
// Sub-nav links for member-gated pages (dao, provider) start hidden and are
// revealed if any wallet in the local vault qualifies.
...
function isActivePage(pathname, href) {
  ...
  // Match /wallet/ as active for /wallet/?addr=… or /wallet (with no trailing slash)
  // but not for /wallet/wallets/ — longer prefixes win below.
...
async function onSearchSubmit(ev) {
  ...
  // Account addresses and block hashes are both 64-hex and indistinguishable by
  // shape, so resolve by probing the API: a block hash resolves at /blocks/{id};
  // otherwise an on-chain account has a non-empty chain. Unresolvable ids report
  // not-found instead of landing on the wrong page. (#785)
  input.disabled = true;
  let dest = null;
...
// apiOrNull returns the parsed body of an API path, or null on any error —
// used to classify an ambiguous search term without throwing.
...
// searchNotFound surfaces a clear message on the search box without navigating.
...
// Single-flight so concurrent callers (e.g. the header poll and a page's own
// startup, which both fire on load) share one GET /node instead of racing two
// (#747). Sequential calls still refetch, which is what a poll wants.
...
// #829: a vault entry now carries two distinct identifiers, and the two gates
// below match against different ones. Reading the wrong field is silent — the
// link simply never appears — so they are collected separately rather than
// through one "addresses" list that either gate might reach for.
...
async function refreshGates() {
  // ADDRESSES identify accounts (providers); PUBLIC KEYS are credentials
  // (the DAO keyset). Before #829 these were the same string.
  const addrs = vaultField('address');
  const pubKeys = vaultField('pub_key');
  if (addrs.length === 0 && pubKeys.length === 0) return;

  // DAO: /statechain/keyset holds VERIFYING KEYS, so match on pub_key, never on
  // address. Entries written before #829 cached the key under `address` and have
  // no pub_key at all; unlocking once at /wallet/ rewrites them
  // (vault.syncWalletIdentity). Mirrors the membership test in dao.js.
  reveal('dao', async () => {
  ...
  // Provider: /providers reports an ACCOUNT ADDRESS, so this one matches on
  // address — correct as-is.
