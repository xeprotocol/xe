// Vault: encrypted multi-wallet storage in localStorage.
//
// Schema (xe.wallets):
//   {
//     wallets: [{ name, address, pub_key, salt, iv, encrypted_seed,
//                 iterations, created_at }],
//     active: <index>
//   }
// #829: `address` and `pub_key` are different values — the address is
// sha256("xe/account/v1" || pubkey). Both are cached here so the locked UI can
// show an address and the DAO page can test keyset membership (which is by
// public key) without prompting for a passphrase. Neither is authoritative:
// the encrypted seed is, and syncWalletIdentity repairs a stale pair on unlock.
// Each seed (32 bytes) is encrypted with AES-GCM using a key derived from the
// shared passphrase + per-wallet salt via PBKDF2-SHA-256.
...
// Encrypt a 32-byte seed with passphrase. Returns wallet entry fields.
...
// Decrypt a wallet entry's seed. Throws on wrong passphrase (AES-GCM auth tag).
...
// Build a fresh wallet entry from name/key pair/seed/passphrase. Takes the key
// pair rather than a bare address so the entry can never record an address and
// a public key that don't belong together (#829).
...
// syncWalletIdentity repairs a vault entry whose cached identity no longer
// matches the seed it holds — which is every entry written before #829, when
// `address` was the raw public key. Returns true if anything changed, so the
// caller knows to persist. The seed is the source of truth; these fields are
// only a cache for the locked UI.
...
// removeWallet splices out the wallet at `index` and fixes the active pointer.
// Deleting an entry *before* the active one shifts every later index down by
// one, so `active` must decrement to keep pointing at the same wallet (the
// #748 bug was clamping only when active ran off the end). Then clamp so the
// pointer stays in range. Returns the removed entry so the caller can purge
// its per-address localStorage keys.
...
//
// On unlock, the passphrase is cached in sessionStorage so subsequent page
// navigations within the same tab don't re-prompt. Cache lives until:
//   - explicit lock,
//   - tab close (sessionStorage is per-tab),
//   - hard TTL of UNLOCK_TTL_MS since unlock.
//
// Storing the passphrase in sessionStorage is roughly equivalent in risk to
// storing the decrypted seed in JS memory — both are accessible to any code
// running in the same origin. The vault on disk remains encrypted regardless.
...
export function cacheUnlock(passphrase) {
  ...
  } catch { /* sessionStorage disabled — degrade silently */ }
}
...
// Try to decrypt the wallet's seed using the cached passphrase, if any.
// Returns the seed bytes on success, or null if no cache / decryption fails.
export async function tryCachedDecrypt(walletEntry) {
  ...
  try {
    ...
    // Cache stale (different vault, wrong passphrase) — drop it.
    clearCachedUnlock();
    return null;
  }
}
