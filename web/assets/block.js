...
// Fields that hold an account ADDRESS and so link to /accounts/. #829: pub_key
// and access_pub_key are deliberately NOT here — they are verifying keys, and
// linking one to an account page would send the reader to a page for an
// account that does not exist. They fall through to the plain 64-hex renderer.
...
// Monetary fields stored as uint64 micro-units. amount/balance follow the
// block's own asset; cost is a lease price, always XUSD.
const AMOUNT_KEYS = new Set(['amount', 'balance', 'cost']);
// Emission-lock economics on lease_accept blocks (#401): ×1000 fixed-point
// and milli-USD — see the milli-scale helpers in format.js.
...
async function load() {
  ...
  // Detail mode when a specific block is requested; otherwise the recent-blocks
  // list. (#789)
  if (hash) {
