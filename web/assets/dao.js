...
const state = {
  ...
  keyset: null,
  vault: null,
  walletIdx: -1,        // index in vault.wallets of the DAO-member wallet
  keyPair: null,        // active member's key pair (after unlock)
  draft: null,          // { index, prev_hash, ops: [...], timestamp, hash }
  signatures: [],       // [{public_key, signature}]
  ops: [{ action: 'set', key: '', value: '' }], // editable until hash generated
  hashLocked: false,
};
...
async function routeStartup() {
  ...
  // Prefer the active wallet if it's a member; otherwise pick the first member
  // wallet found in the vault. #829: a keyset holds PUBLIC KEYS, so match on
  // the vault entry's pub_key, not its address. Entries written before #829
  // cached the key under `address` and carry no pub_key at all — unlocking
  // once at /wallet/ rewrites them (vault.syncWalletIdentity).
  ...
  // Try cached unlock from this tab session before prompting.
...
async function showDAO() {
  ...
  // #829: a DAO keyset holds raw ed25519 PUBLIC KEYS, not account addresses —
  // membership and the state-chain signature list are both keyed by pubKeyHex.
...
async function onGenerateHash() {
  ...
  state.draft = {
    ...
    timestamp: Date.now() * 1_000_000, // milliseconds → nanoseconds
  };
...
async function onSignSelf() {
  ...
  // public_key on a state-chain signature is a raw verifying key (#829).
...
async function onImportSig(ev) {
  ...
  // Verify against current draft hash via Web Crypto.
  try {
...
async function onSubmitBlock() {
  ...
  try {
    ...
    onResetDraft();
    // Refresh tip so the next draft chains correctly.
    state.tip = await api('/statechain/tip');
    setText('tip-index', state.tip.index);
    setText('tip-hash', state.tip.hash);
  } catch (e) {
