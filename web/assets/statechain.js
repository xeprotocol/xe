...
async function onKvLookup(ev) {
  ...
  try {
    ...
    // Definitive miss on the exact route → fall back to a prefix search on
    // the list endpoint (#328), so the form delivers the prefix behaviour it
    // advertises. Any other failure (network, 5xx) is a real error and must
    // surface, not be masked by a prefix listing — and a failing fallback
    // reports its own error rather than claiming the key doesn't exist.
