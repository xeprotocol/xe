...
const CONTACTS_PREFIX = 'xe.chat.contacts.'; // appended with active wallet addr
...
const state = {
  ...
  nodePeer: '',
  networkID: '',
  contacts: new Set(),       // strings
  active: null,               // active contact addr
  messages: [],               // all envelopes for self
  unread: new Map(),          // contact addr → count
  registered: new Map(),      // contact addr → bool (directory-resolved, #746)
  sse: null,
  sseReconnect: null,         // pending reconnect timer
  syncTimer: null,            // periodic catch-up re-sync (#744)
  dirTimer: null,             // periodic directory re-register (#743)
};

// The directory is a TTL'd in-memory cache (default 30 min). Re-register well
// under the TTL so an open tab never drops out, and so a node restart (which
// wipes registrations) is recovered within one interval (#743).
const DIRECTORY_REFRESH_MS = 10 * 60 * 1000;

// Periodic full re-sync interval. The listener buffer is a 64-slot
// non-blocking channel (server drops on overflow), so a message can be missed
// mid-stream without any reconnect. A periodic loadMessages() (id-deduped)
// backfills those gaps (#744).
...
async function init() {
  ...
  state.walletIdx = state.vault.active >= 0 && state.vault.active < state.vault.wallets.length
    ? state.vault.active
    : 0;
  // Try cached unlock from this tab session before prompting. Guard the
  // key-load + chat-open so a transient crypto failure falls back to the
  // unlock prompt instead of a blank screen (#746).
...
// Reuse nav's single-flight refreshNode (it already owns the shared header's
// network/version/status) instead of a second GET /node; just capture the
// node identity this page needs (#747).
...
async function onUnlockSubmit(ev) {
  ...
  // The steps after decryption were previously unguarded: on a browser without
  // WebCrypto Ed25519 (or any transient crypto failure) the correct passphrase
  // cleared the field and silently did nothing. Surface it (#746).
  try {
    state.keyPair = await loadKeyPair(seed);
    cacheUnlock(pass);
    await showChat();
...
// scheduleDirectoryRefresh keeps this wallet's directory entry alive by
// re-registering under the TTL. Clears any prior timer first so re-unlock
// cycles don't stack timers (#743).
...
// scheduleChatSync periodically re-fetches messages to backfill anything the
// live stream dropped (buffer overflow, or the load-vs-subscribe race).
// appendEnvelope dedupes by id, so a full re-fetch is safe. Clears any prior
// timer so re-unlock cycles don't stack timers (#744).
...
async function registerInDirectory() {
  ...
  try {
    ...
    // #829: the body must declare pub_key alongside the account address —
    // buildRegistrationJson is the path that cannot omit it.
...
function onAddContact(ev) {
  ...
  showError('');
  resolveRecipient(addr); // advise if not registered (#746)
  if (!state.active) selectContact(addr);
  else renderContacts();
}

function deriveContactsFromMessages() {
  // Add anyone we've messaged with to contacts (auto-discover).
...
// resolveRecipient checks whether addr is registered anywhere in the directory
// and caches the result. A negative is advisory, not authoritative — the
// directory is registered-via-gossip, not live presence — so the server guard
// on send stays the source of truth (#746). Refreshes the contact pills and,
// if addr is the active contact, the advisory note.
async function resolveRecipient(addr) {
  try {
    ...
    // Network error — leave the entry unknown rather than asserting a negative.
    return;
  }
  renderContacts();
...
async function onSend(ev) {
  ...
  try {
    // buildEnvelope solves the anti-spam PoW client-side — typically well
    // under a second, but keep the button honest while it grinds.
    ...
    if (!res.ok) {
      ...
      if (res.status === 404) {
        // Recipient isn't registered anywhere the node knows — non-fatal, but
        // the message won't be delivered (#746).
    ...
    // Optimistically add to local state in case the SSE stream is slow.
    appendEnvelope({
      id: env.id,
      from: env.from,
      to: env.to,
      message: env.message,
      timestamp: Number(env.timestamp), // for sort/display only
...
function appendEnvelope(env) {
  if (state.messages.find((m) => m.id === env.id)) return; // dedupe
...
async function startSSE() {
  ...
  // The event stream requires an ownership proof (#745). The challenge is
  // single-use, so mint a fresh one per connection attempt.
  let proof;
  try {
  ...
  es.onerror = () => {
    // A consumed single-use challenge means EventSource's own reconnect would
    // be rejected, so take over reconnection — each attempt mints a fresh proof.
    // On a fatal error the browser has already set readyState CLOSED and will
    // not retry on its own; distinguish it so the status isn't a lie (#744).
  ...
  es.onopen = () => {
    setStatus('ok', 'ok');
    // Catch up on anything missed before this (re)connection was live — the
    // initial load-vs-subscribe race and every reconnect gap. Deduped by id.
    loadMessages();
  };
  state.sse = es;
}
...
// errorDetail extracts a human message from an error response: the JSON
// `.error` field the API returns, falling back to the raw body or status code.
// Avoids dumping a raw JSON blob into the banner (#746).
...
// showInfo shows a soft, non-blocking advisory in the polite #info region —
// used for the "recipient not registered" hint (#746), not for errors.
