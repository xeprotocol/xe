// Tiny SVG chart primitives. No deps. Charts use a fixed internal viewBox
// (480×120) and scale to their parent via width=100% / height=100% in CSS.
...
// chartWrap returns a positioned div containing the (stretching) SVG and
// pixel-fixed HTML axis labels on top. Without this, a `preserveAspectRatio:
// none` SVG distorts text horizontally when the container is much wider than
// the 480-unit viewBox.
...
// baseline draws a faint x-axis rule so an all-zero chart still shows
...
// samples: [{x: number, y: number}], rendered as a polyline.
...
// Compact inline sparkline. Designed for stat-card use (~120×28 in CSS).
// values: number[]; rendered as a polyline. No axes, no padding.
...
// values: number[], rendered as bars filling the chart width.
