...
//
// Returns a retained certificate by hash, including expired ones. Lease blocks
// pin a certificate hash, so this is how an operator (or a live test) checks
// that the certificate backing historical lease blocks is still held after the
// provider has rotated — the condition #816 was about. Registered before the
// single-segment provider route below; the two patterns do not overlap.
...
//
// The provider's current *usable* certificate. Expired certificates are not
// served here even when retained — a consumer would write a lease the ledger
// rejects. Use /certificate/hash/{hash} to reach retained history.
...
func (h *Handler) handleLeaseRequest(w http.ResponseWriter, r *http.Request) {
	...
	// Reject anything the chain would reject before spending a marketplace
	// round-trip and a proof-of-work on it (#808/#809).
