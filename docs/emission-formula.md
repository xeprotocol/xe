...
minute** (#805): the duration is rounded up to whole minutes and the hourly
rate is divided down by 60. `LeaseMinDuration` is 60s, so the smallest legal
lease is exactly one billing unit.

Whole-hour durations price identically to the pre-#805 hour-granular scheme —
...
Before #805 the same lease was quantised up to a whole hour and cost 31,000
...
- **R effective**: from the state chain epoch data (`r_effective`). Since
  #401, emission parameters are locked onto the lease record at accept time
