...
- **Auth.** The public API is unauthenticated **except** operator-only endpoints (currently `POST /lease/request`), which require `Authorization: Bearer <token>` where the token is set via `XE_API_ADMIN_TOKEN`. If no token is configured the endpoint returns `403`.

### Addresses vs. keys

An account **address** is no longer its ed25519 public key (#829). It is the
...
multisig addresses have always had, and the precondition for key rotation
(#424). A multisig account's address is its keyset digest, not a public key, so
...
| GET | `/chat/contacts` | Known chat contacts |
...
Three endpoints, three different questions (#841). Confusing them is itself an
...
| `xe_quorum_margin_ratio` | gauge | `active ÷ total − 0.67`. Below 0 every position falls to the stall fallback |
...
| `xe_finality_advances_total` | counter | Final-height watermark advances since start, read straight off the ledger's own counter (#833) rather than re-derived. Flat forever means nothing is finalizing |
...
| `xe_delegation_underflows` | gauge | Vote-weight underflows; any non-zero value is a correctness fault (#728) |
| `xe_ineligible_weight_micro_xe` | gauge | Weight the representative-eligibility policy holds out of the quorum denominator (#832). Zero when unrestricted |
| `xe_rep_eligibility_restricted` | gauge | 1 when a `sys.representatives` allowlist is actually in force |
| `xe_rep_eligibility_failed_open` | gauge | 1 when a published allowlist matched **zero** delegated weight and is being ignored (#832) — the policy is not being enforced |
| `xe_accounts_total` / `xe_blocks_total` / `xe_pending_sends` | gauge | Ledger size |
| `xe_supply_micro_units{asset}` | gauge | Circulating supply (`observed` = balances plus in-flight sends), from the `GET /supply` auditor (#837) — not a second derivation |
| `xe_supply_conserved{asset}` | gauge | 1 when this node's supply identity balances, 0 when it does not (#837). Zero is a supply-conservation breach |
...
| `xe_dropped_blocks_total` | counter | Blocks shed from a full gossip receive channel (#731) |
...
file are committed under `deploy/monitoring/`. The incident procedures for every
alert are in [`docs/runbook.md`](runbook.md).
...
```
> `balances` includes unfinalized inflows; **`spendable` counts only finalized (irreversible) funds — treat it as the settlement figure** (#528). `final_height` is the account's finalized-block watermark.
...
> declares a block `pub_key` (#829). Errors: `400 missing address`,
...
when a lease's escrow is burnt (at settle, or at #493 escrow expiry) or a provider's
...
> Continuous auditor: `scripts/audit-supply` recomputes this identity from the primitive
> endpoints, asserts `/supply` agrees term by term, and fails loudly the moment it breaks.
...
#### `pub_key`: the opening-block key declaration (#829)
...
no key is declared, so a non-opening block hashes exactly as it did before #829.
...
- `lease_accept` — provider-signed; carries `certificate_hash`, `attestations`, and the locked emission params `locked_r` / `locked_payout_cap` / `locked_twap_milli` (#501).
...
- `mint` — **XUSD, authorized minter only** (`sys.minter`, #519); rejected otherwise. `sys.minter.keys` holds account **addresses**, matched against the block's `account` (unlike `sys.timekeepers` / `sys.oracle` / `sys.dao_keyset`, which hold verifying keys matched against signatures) (#829).
...
| `account` | string | account **address**, 64 hex — `sha256("xe/account/v1" \|\| pubkey)`, or a multisig keyset digest. **Not** a public key (#829) |
| `previous` | string | prev block hash; `"0"` for first block |
| `pub_key` | string | account's ed25519 **public key**, 64 hex. **Required** iff `previous == "0"` on a single-key chain; **forbidden** on every other block and on all multisig blocks (#829) |
...
| `lease_min_duration_secs`,`lease_settle_grace_ns`,`lease_force_settle_gap_ns`,`lease_escrow_expiry_ns`,`lease_archive_gap_ns`,`max_attestation_skew_ns` | uint64/int64 | **genesis block only** (#524/#662) |

---

## PoW

Proof-of-work is solved **client-side** (#729). The old `POST /pow` endpoint — which ground nonces on the node's CPU for any 64-hex hash, an offload/DoS oracle usable to bill block *and* chat PoW to the victim — is removed. To solve: compute the 8-byte unkeyed blake2b digest of `nonce_LE(8 bytes) || hash(32 bytes)`, read it as a big-endian uint64, and search for a nonce where the value is `>=` the difficulty. Difficulties are advertised by `GET /node` as hex strings: `pow_difficulty` (block submission) and `chat_pow_difficulty` (chat envelopes, solved over the envelope `id`). The web UI ships a reference implementation in `web/assets/blake2b.js` + `web/assets/pow.js`.
...
### GET /delegation
Representative vote weights in **micro-XE** (#515), split by eligibility for the
quorum denominator (#832). No params.
```json
{
  "weights": {
...
> but weight is delegated to and tallied against the derived address (#829).
...
> not an account address (#829). Errors: `400 invalid request body`,
...
Register (or refresh) your own account → peer mapping. Body: a signed `Registration` (`account`, `pub_key`, `node_peer`, `timestamp`, `signature`). Response `200 {"ok":true}`. Errors: `400 invalid request body` / signature-validation error.

`pub_key` is **required** (#829): `account` is an address, so the record no
...
`pub_key` is **required** (#829), for the same reason as on a directory
...
### GET /chat/messages
Fetch chat messages for an account. Query: `account` (**required**), `since` (optional, unix nanos, default 0). Returns an array of envelopes. Errors: `400 missing account parameter`.

### GET /chat/contacts
Accounts this node has chat history with. No params. Returns an array of account strings.

### GET /chat/events
Server-Sent Events stream of incoming chat. `Content-Type: text/event-stream`; each event is `data: <envelope json>\n\n`. Query: `account` (optional) filters to envelopes where `from==account || to==account`. Errors: `500 streaming not supported`.
> On connect the handler immediately flushes an SSE comment (`: connected`) so the response headers and `text/event-stream` content-type reach the client before any message arrives — this lets `EventSource.onopen` fire and lets the reverse proxy disable response buffering — then sends a `: ping` comment every 20s to keep idle connections alive and detect dead peers (#698).

---

## Out of scope (separate services, own APIs)

The faucet, oracle, and explorer are separate services with their own small HTTP surfaces — documented in their own tickets/repos. This reference covers the **core node** API only.
