...
func (gw *SSHGateway) handleConnection(conn net.Conn) {
	...
	config := &ssh.ServerConfig{
		PublicKeyCallback: func(connMeta ssh.ConnMetadata, key ssh.PublicKey) (*ssh.Permissions, error) {
			...
			// Access requires a live lease — accepted is the only state with
			// a running VM. The legacy Settled bool stays false for cancelled
			// leases (#761).
...
// handleDirectTCPIP handles direct-tcpip channels (ssh -W / ProxyJump).
// The client's SSH negotiates end-to-end with the VM's sshd through this tunnel.
...
func (gw *SSHGateway) handleSession(channel ssh.Channel, requests <-chan *ssh.Request, lease *core.Lease) {
	...
	// Create a context that is cancelled when the session ends (channel closes
	// or gateway shuts down), so the tunnel is torn down with the session.
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	// Cancel context when gateway is shutting down.
	go func() {
		select {
		case <-gw.done:
			cancel()
