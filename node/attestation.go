...
const (
	...
	// AttestationRateLimit is the minimum interval between attestation
	// requests from the same peer for the same lease.
...
// canAttest reports whether this node may sign a timestamp attestation for
// identifier (#570/H8). A signature is only handed out for:
//   - a lease the node holds that can still take attestation-gated blocks
//     (accept needs created; settle/force-settle need accepted), or
//   - a performance-certificate benchmark for a directory-registered
//     provider: the provider address itself (start) or that address with the
//     middle 16 hex chars zeroed (end — see generatePerformanceCertificate).
//
// Anything else would make every timekeeper a free-form signing oracle:
// harvested attestations pass ValidateAttestations inside attacker-crafted
// accept/force-settle blocks.
...
func (n *Node) registerAttestationHandler() {
	n.attRateMap = make(map[string]time.Time) // "peerID:leaseHash" → last request time
	n.Msg.Handle(AttestationMsgType, n.handleAttestationRequest)
}

// handleAttestationRequest serves a timekeeper attestation over the
// Messenger. Named (not a closure) so the rate-limit ordering is testable
// (#634): the per-identifier slot is recorded only after validation passes.
func (n *Node) handleAttestationRequest(from peer.ID, payload json.RawMessage) (json.RawMessage, error) {
	...
	// Rate limit: one attestation per peer per lease per interval.
	// Evict expired entries on each access to prevent unbounded growth.
	...
	// Validate identifier: must be a 64-char hex string (lease hash or
	// provider address for performance certificates).
	...
	if err := n.canAttest(req.LeaseHash); err != nil {
		// Visible server-side: rejections previously surfaced only to the
		// requester, which made #634 diagnosable only from client logs.
		logging.Warnf("Attestation request from %s rejected: %v", from.ShortString(), err)
		return nil, err
	}

	// Record the rate-limit slot only now, on the way to actually signing.
	// Recording it before validation burned the 30s window on rejections —
	// a request that merely raced lease propagation (#634) locked the
	// identifier out on this timekeeper, so the retry that would have
	// succeeded got "rate limited" instead. The limiter guards the signing
	// oracle (#618/H8); rejected requests sign nothing.
	n.attRateMu.Lock()
...
func (n *Node) gatherAttestations(leaseHash string, config *core.TimekeeperConfig) ([]core.TimekeeperAttestation, error) {
	...
	// Also sign locally if this node is a timekeeper.
	...
	// Request from all connected peers in parallel.
	for _, p := range peers {
		wg.Add(1)
		go func(pid peer.ID) {
			defer wg.Done()
...
// capAttestations keeps only valid, distinct trusted-timekeeper attestations
// and caps the result at MaxAttestationsPerBlock. Every node answers
// attestation requests regardless of timekeeper membership, and post-#596 the
// set is bound into the block hash with ValidateAttestations rejecting
// oversized sets — so without this a provider with >MaxAttestationsPerBlock
// connected peers built accept/settle blocks that failed its own validation
// (#599). Untrusted/duplicate/invalid entries never count toward the
// threshold, so dropping them here loses nothing.
